...
import com.danby.happynode.data.align.model.dto.NoteOperateMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "happynode_group_data_align_" + MQConstants.TOPIC_NOTE_OPERATE, // Group 组
...
public class TodayNotePublishIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        // 执行 Lua 脚本，拿到返回结果
        Long result = redisTemplate.execute(script, Collections.singletonList(bloomUserNoteOperateListKey), noteId);
        // 若布隆过滤器判断不存在（绝对正确）
        if (Objects.equals(result, 0L)) {
            ...
            redisTemplate.execute(bloomAddScript, Collections.singletonList(bloomUserNoteOperateListKey), noteCreatorId);
        }
