...
@Component
@RocketMQMessageListener(consumerGroup = "happymode_group_data_align_" + MQConstants.TOPIC_COUNT_NOTE_LIKE, // Group 组
...
public class TodayNoteLikeIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        // ------------------------- 笔记的点赞数变更记录 -------------------------
        // 布隆过滤器key
        String bloomKey = RedisKeyConstants.buildBloomUserNoteLikeNoteIdListKey(date);
        ...
        // 执行 Lua 脚本，拿到返回结果
        result = redisTemplate.execute(script, Collections.singletonList(bloomKey), noteCreatorId);
        // 若布隆过滤器判断不存在（绝对正确）
        if (Objects.equals(result, 0L)) {
            // 2. 若无，才会落库，减轻数据库压力
            // 根据分片总数，取模，分别获取对应的分片序号
