...
public class ParamUtils {
     ...
     * @return
     */
    public static boolean checkHappynodeId(String happynodeId) {
        // 检查长度
        if (happynodeId.length() < ID_MIN_LENGTH || happynodeId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(happynodeId).matches();
    }
