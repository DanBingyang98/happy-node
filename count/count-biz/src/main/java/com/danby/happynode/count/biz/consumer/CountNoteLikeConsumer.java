...
import com.danby.happynode.count.biz.enums.LikeUnlikeNoteTypeEnum;
import com.danby.happynode.count.biz.model.dto.AggregationCountLikeUnlikeNoteMqDTO;
import com.danby.happynode.count.biz.model.dto.CountLikeUnlikeNoteMqDTO;
...
@RocketMQMessageListener(
        consumerGroup = "happynode_group_" + MQConstants.TOPIC_COUNT_NOTE_LIKE, // group 组
...
public class CountNoteLikeConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        for (Map.Entry<Long, List<CountLikeUnlikeNoteMqDTO>> entry : groupMap.entrySet()) {
            ...
            // 笔记发布者 ID
            Long noteCreatorId = null;
            // 最终的计数值，默认为 0
            Integer finalCount = 0;
            for (CountLikeUnlikeNoteMqDTO countLikeUnlikeNoteMqDTO : entry.getValue()) {
                noteCreatorId = countLikeUnlikeNoteMqDTO.getNoteCreatorId();
            ...
            countList.add(AggregationCountLikeUnlikeNoteMqDTO.builder()
                    ...
                    .count(finalCount)
                    .build());
//            noteCreatorId.put(noteId, finalCount);
        }
        log.info("## 【笔记点赞数】聚合后的计数数据: {}", JsonUtils.toJsonString(countList));
        // 更新 Redis
        countList.forEach((aggregationCountLikeUnlikeNoteMqDTO) -> {
            String redisKey = RedisKeyConstants.buildCountNoteKey(aggregationCountLikeUnlikeNoteMqDTO.getNoteId());
            ...
            if (isExisted) {
                ...
                redisTemplate.opsForHash().increment(redisKey, RedisKeyConstants.FIELD_LIKE_TOTAL, aggregationCountLikeUnlikeNoteMqDTO.getCount());
            }
            // 更新 Redis 用户维度点赞数
            String countUserRedisKey = RedisKeyConstants.buildCountUserKey(aggregationCountLikeUnlikeNoteMqDTO.getCreatorId());
            boolean isCountUserExisted = redisTemplate.hasKey(countUserRedisKey);
            if (isCountUserExisted) {
                redisTemplate.opsForHash().increment(countUserRedisKey, RedisKeyConstants.FIELD_LIKE_TOTAL, aggregationCountLikeUnlikeNoteMqDTO.getCount());
            }
        });
        // 发送 MQ, 笔记点赞数据落库
