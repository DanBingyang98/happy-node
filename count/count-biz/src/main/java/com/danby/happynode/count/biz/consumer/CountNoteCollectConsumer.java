...
import com.danby.happynode.count.biz.enums.CollectUnCollectNoteTypeEnum;
...
@RocketMQMessageListener(
        consumerGroup = "happynode_group_" + MQConstants.TOPIC_COUNT_NOTE_COLLECT,
...
public class CountNoteCollectConsumer implements RocketMQListener<String> {
    ...
    private BufferTrigger<String> bufferTrigger = BufferTrigger.<String>batchBlocking()
            ...
            .batchSize(1000) // 一个批次最多聚合 1000 条
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        for (Map.Entry<Long, List<CountCollectUnCollectNoteMqDTO>> entry : groupMap.entrySet()) {
            ...
            int finalCount = 0;
            for (CountCollectUnCollectNoteMqDTO countCollectUnCollectNoteMqDTO : entry.getValue()) {
                // 获取操作类型
                int type = countCollectUnCollectNoteMqDTO.getType();
                // 根据操作类型，获取对应枚举
            ...
            countMap.put(noteId, finalCount);
        }
        ...
        countMap.forEach((noteId, count) -> {
            ...
            if (hasKey) {
                ...
                redisTemplate.opsForHash().increment(countNoteKey, RedisKeyConstants.FIELD_COLLECT_TOTAL, count);
            }
        });
        // 发送 MQ, 笔记收藏数据落库
